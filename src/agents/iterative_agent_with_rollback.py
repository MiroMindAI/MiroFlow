...
class IterativeAgentWithToolAndRollback(BaseAgent):
    ...
    def _should_rollback(
        ...
        try:
            if (llm_output.raw_response and 
                llm_output.raw_response.choices and
                len(llm_output.raw_response.choices) > 0 and
                llm_output.raw_response.choices[0].finish_reason == "length"):
    ...
    async def run_internal(self, ctx: AgentContext) -> AgentContext:
        ...
        while max_turns == -1 or turn_count < max_turns:
            ...
            should_rollback, rollback_reason = self._should_rollback(
                llm_output, 
                tool_and_sub_agent_calls, 
                llm_output.response_text
            )
            
            if len(tool_and_sub_agent_calls) == 0:
                if should_rollback and consecutive_rollbacks < self.max_consecutive_rollbacks:
                ...
                tool_calls = [
                    call for call in tool_and_sub_agent_calls 
                    if ('agent-worker' not in call['server_name'] and 
                        'skills-worker' not in call['server_name'])
                ]
                sub_agent_calls = [
                    call for call in tool_and_sub_agent_calls 
