...
class DeepSeekOpenRouterClient(LLMProviderClientBase):
    def _create_client(self, config: DictConfig):
        """Create configured OpenAI client for DeepSeek via OpenRouter"""
    ...
    async def _create_message(
        ...
        try:
            ...
            if (
                ...
                logger.debug(f"DeepSeek OpenRouter LLM Context limit exceeded: {error_str}")
                raise ContextLimitError(f"Context limit exceeded: {error_str}")

            logger.error(
                f"DeepSeek OpenRouter LLM call failed: {str(e)}, input = {json.dumps(params)}",
                exc_info=True,
            )
            raise e
    ...
    def extract_tool_calls_info(self, llm_response, assistant_response_text):
        """Extract tool call information from DeepSeek LLM response"""
        from src.utils.parsing_utils import parse_llm_response_for_tool_calls

        # For OpenAI-compatible APIs, directly get tool calls from response object
    ...
    def parse_llm_response(self, llm_response) -> str:
        """Parse DeepSeek LLM response to get text content"""
        if not llm_response or not llm_response.choices:
